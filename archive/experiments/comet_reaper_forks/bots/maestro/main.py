...
class ProducerLiteConfig:
    ...
    # --- maestro: standing-aware roi_threshold controller ----------------
    # The family runs a fixed roi_threshold (+ comet_reaper's terminal surge).
    # maestro modulates it by our standing each turn: AHEAD ⇒ raise roi (bank the
    # lead, fewer risky attacks); BEHIND ⇒ lower roi (press to catch up). This is
    # the v2-gru controller's core heuristic, distilled. 0 ⇒ comet_reaper.
    maestro_gain: float = 0.25
...
def _maestro_modulate(config: ProducerLiteConfig, obs, player_count: int) -> ProducerLiteConfig:
    """Standing-aware roi_threshold: ahead ⇒ raise (play safe), behind ⇒ lower (press).

    Strength proxy = total alive ships per player; ratio = mine / strongest rival.
    ``maestro_gain`` scales the response; 0 ⇒ unchanged (comet_reaper). Crash-safe.
    """
    g = float(getattr(config, "maestro_gain", 0.0))
    if g <= 0.0:
        return config
    try:
        owner = obs.owner_abs.to(torch.long)
        ships = obs.ships.to(torch.float32)
        pid = int(obs.player_id)
        tot = [float(ships[obs.alive & (owner == o)].sum()) for o in range(int(player_count))]
        me = tot[pid] if 0 <= pid < len(tot) else 0.0
        rivals = [tot[o] for o in range(int(player_count)) if o != pid]
        ratio = me / max(max(rivals) if rivals else 0.0, 1e-6)
        delta = max(-0.4, min((ratio - 1.0) * g, 0.4))                # ahead→+, behind→−
        roi = max(1.0, min(float(config.roi_threshold) * (1.0 + delta), 2.2))
        return dataclasses.replace(config, roi_threshold=roi)
    except Exception:
        return config
...
def run_turn(obs_tensors: dict, *, config: ProducerLiteConfig, player_count: int, memory) -> dict:
    ...
    config = _maestro_modulate(config, obs, int(player_count))   # standing-aware roi_threshold
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        _g = os.environ.get("MAESTRO_GAIN")        # knob sweep via env (tuning loop)
        if _g is not None:
            config = dataclasses.replace(config, maestro_gain=float(_g))
        row = run_turn(
