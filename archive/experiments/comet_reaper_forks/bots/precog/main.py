...
from orbit_lite.garrison_launch import LaunchSet  # precog: mixed-owner what-if launch sets
...
class ProducerLiteConfig:
    ...
    # --- precog: opponent-response modeling ------------------------------
    # The whole orbit_lite family scores my waves against a do-nothing opponent
    # projection (ORBIT_LITE_FAMILY.md's #1 shared blind spot). precog predicts
    # each rival's likely launches this turn (cheap safe_drain proxy) and scores
    # my candidates by MARGINAL competitive value vs a *moving* opponent:
    #   score = score(my_wave + predicted_opp) - score(predicted_opp alone).
    # opp_model_strength scales predicted rival ship commitment; 0.0 ⇒ identical
    # to comet_reaper. Any internal error falls back to the do-nothing score.
    opp_model_strength: float = 1.0
    opp_model_max_sources: int = 8          # cap predicted launches per rival
...
def _predict_opponent_launches(obs, garrison_status, cache, *, config, H, K_eta, pid, player_count):
    """Cheap per-rival launch prediction for precog's opponent-response scoring.

    For each rival: take its strongest sources (garrison ≥ ``min_ships_to_launch``),
    compute its ``safe_drain`` surplus over the horizon (scaled by
    ``opp_model_strength``), and predict it sends that surplus at its nearest
    non-owned planet, arriving at a distance/​speed-based ETA (the flow projector
    needs only src/tgt/ships/eta/owner — not the firing angle). Returns LaunchSet
    field tensors ``[L]`` (owner = the predicting rival), or ``None`` if nobody can
    launch. Pure tensor arithmetic on already-built caches — no extra planning pass.
    """
    device = obs.device
    dtype = obs.ships.dtype
    P = int(obs.P)
    s = float(config.opp_model_strength)
    if P == 0 or s <= 0.0:
        return None
    ...
    alive = obs.alive
    ships = obs.ships.to(dtype)
    min_ships = float(config.min_ships_to_launch)
    H_eff = torch.full((), float(H), dtype=dtype, device=device)
    cap = max(1, int(config.opp_model_max_sources))
    arangeP = torch.arange(P, device=device)

    src_l, tgt_l, ship_l, eta_l, own_l = [], [], [], [], []
    for o in range(int(player_count)):
        if o == int(pid):
            continue
        o_mask = alive & (owner == o) & (ships >= min_ships)
        if not bool(o_mask.any()):
            continue
        o_src = torch.nonzero(o_mask, as_tuple=False).flatten()
        if int(o_src.numel()) > cap:                            # strongest sources only
            o_src = o_src[torch.topk(ships[o_src], cap).indices]
        drain = safe_drain(garrison_status, source_idx=o_src, source_ships=ships[o_src],
                           H_eff=H_eff, player_id=o)            # [So]
        send = (drain * s).floor()
        notmine = (alive & (owner != o)).view(1, P)             # [1, P]
        dist = torch.where(notmine, d0[o_src], torch.full_like(d0[o_src], float("inf")))  # [So, P]
        dist = dist.scatter(1, o_src.view(-1, 1), float("inf"))  # never target self
        tgt = dist.argmin(dim=1)                                # [So]
        tdist = dist.gather(1, tgt.view(-1, 1)).squeeze(1)      # [So]
        speed = fleet_speed(send.clamp(min=1.0))                # [So]
        eta = torch.ceil(tdist / speed.clamp(min=1e-6)).clamp(min=1.0, max=float(K_eta))
        ok = torch.isfinite(tdist) & (send >= 1.0)
        for i in range(int(o_src.numel())):
            if bool(ok[i]):
                src_l.append(int(o_src[i])); tgt_l.append(int(tgt[i]))
                ship_l.append(float(send[i])); eta_l.append(float(eta[i])); own_l.append(o)
    if not src_l:
        return None
    L = len(src_l)
    return {
        "source_slots": torch.tensor(src_l, dtype=torch.long, device=device),
        "target_slots": torch.tensor(tgt_l, dtype=torch.long, device=device),
        "ships": torch.tensor(ship_l, dtype=dtype, device=device),
        "eta": torch.tensor(eta_l, dtype=dtype, device=device),
        "owner": torch.tensor(own_l, dtype=torch.long, device=device),
...
def plan_lite_waves(
    *,
    ...
    # precog: score each candidate against a board where rivals ALSO move this turn.
    # marginal value = score(my_wave + predicted_opp) - score(predicted_opp alone),
    # isolating my contribution vs a *moving* opponent. Any error → do-nothing score.
    try:
        opp = _predict_opponent_launches(
            obs, garrison_status, cache,
            config=config, H=H, K_eta=K_eta, pid=pid, player_count=int(player_count),
        )
    except Exception:
        opp = None
    if opp is None:
        ...
        try:
            Lo = int(opp["source_slots"].shape[0])
            _b = lambda t: t.view(1, Lo).expand(C, Lo)         # broadcast [Lo] -> [C, Lo]
            own_owner = torch.full((C, L), int(pid), dtype=torch.long, device=device)
            combined = LaunchSet(
                source_slots=torch.cat([cand_src, _b(opp["source_slots"])], dim=1),
                target_slots=torch.cat([cand_tgt_slot.unsqueeze(-1).expand(C, L), _b(opp["target_slots"])], dim=1),
                ships=torch.cat([cand_send, _b(opp["ships"])], dim=1),
                eta=torch.cat([cand_eta, _b(opp["eta"])], dim=1),
                owner=torch.cat([own_owner, _b(opp["owner"])], dim=1),
                valid=torch.cat([cand_active & cand_valid.unsqueeze(-1), _b(opp["valid"])], dim=1),
            )
            score_with = score_candidates(
                garrison_status, prod=prod, alive_by_step=alive_by_step,
                player_count=int(player_count), launches=combined, player_id=pid,
            )                                                                    # [C]
            opp_only = LaunchSet(
                source_slots=opp["source_slots"], target_slots=opp["target_slots"],
                ships=opp["ships"], eta=opp["eta"], owner=opp["owner"], valid=opp["valid"],
            )
            score_opp = score_candidates(
                garrison_status, prod=prod, alive_by_step=alive_by_step,
                player_count=int(player_count), launches=opp_only, player_id=pid,
            )                                                                    # scalar
            score = score_with - score_opp
        except Exception:
            score = _base_score()
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        _s = os.environ.get("PRECOG_OPP_STRENGTH")  # knob sweep via env (tuning loop)
        if _s is not None:
            config = dataclasses.replace(config, opp_model_strength=float(_s))
        row = run_turn(
