...
from orbit_lite.garrison_launch import sparse_launch_flow_delta  # kingmaker: weighted competitive score
...
class ProducerLiteConfig:
    ...
    # --- kingmaker: 4P coalition / leader-weighting ----------------------
    # The family scores 4P as an equal-weight opponent sum (competitive_score:
    # me - Σ_opp), so it plays FFA shallow. kingmaker reweights that sum (4P only):
    # overweight the current LEADER's net (prefer plays that hurt whoever's winning)
    # and discount trailing rivals (let them bleed each other). 0 ⇒ comet_reaper.
    leader_weight: float = 0.35             # extra weight on the leader's net
    bleed_discount: float = 0.15            # weight removed from non-leader rivals
...
def plan_lite_waves(
    *,
    ...
    # kingmaker (4P only): reweight the opponent term — overweight the LEADER, discount
    # trailing rivals. score = me - Σ_o w_o·net_o. 2P / disabled -> equal-weight score.
    lw = float(config.leader_weight); bd = float(config.bleed_discount)
    if int(player_count) >= 4 and (lw != 0.0 or bd != 0.0):
        try:
            diff = sparse_launch_flow_delta(
                garrison_status, prod=prod, alive_by_step=alive_by_step,
                player_count=int(player_count), launches=launches, player_id=pid,
            )
            net = diff.net_ship_delta                                            # [C, A]
            A = int(net.shape[-1])
            owner = obs.owner_abs.to(torch.long)
            strength = torch.zeros(A, dtype=dtype, device=device)
            for oid in range(A):                                                 # prod + small ship weight
                m = obs.alive & (owner == oid)
                strength[oid] = prod[m].to(dtype).sum() + 0.025 * obs.ships[m].to(dtype).sum()
            w = torch.ones(A, dtype=dtype, device=device)
            opp_ids = [o for o in range(A) if o != int(pid)]
            if opp_ids:
                leader = max(opp_ids, key=lambda o: float(strength[o]))
                for o in opp_ids:
                    w[o] = (1.0 + lw) if o == leader else (1.0 - bd)
            me = net[..., int(pid)]
            score = me - ((net * w).sum(dim=-1) - me)                            # me - Σ_o w_o·net_o
        except Exception:
            score = _base_score()
    else:
        score = _base_score()
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        _lw = os.environ.get("KINGMAKER_LEADER_WEIGHT")   # knob sweep via env (tuning loop)
        _bd = os.environ.get("KINGMAKER_BLEED")
        if _lw is not None or _bd is not None:
            config = dataclasses.replace(
                config,
                leader_weight=float(_lw) if _lw is not None else config.leader_weight,
                bleed_discount=float(_bd) if _bd is not None else config.bleed_discount,
            )
        row = run_turn(
