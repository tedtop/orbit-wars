...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        # helmsman: look further ahead — extend the projection horizon (attacks the
        # family's static-horizon blind spot: long multi-turn setups are invisible at
        # H=18). Multiplier is env-tunable; 1.0 ⇒ comet_reaper. Bounded ⇒ no timeout.
        try:
            _m = float(os.environ.get("HELMSMAN_HORIZON_MULT", "1.0"))  # sweep: >1.0 regresses (H=18 already tuned); default to parity
            _h = max(8, min(int(round(int(config.horizon) * _m)), 40))
            config = dataclasses.replace(config, horizon=_h)
        except Exception:
            pass
        row = run_turn(
