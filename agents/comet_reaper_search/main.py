...
def plan_lite_waves(
    *,
    ...
    # comet_reaper_search: blend a forward-sim lookahead value into candidate scores. For
    # the top-N candidates by 1-ply score, run a fast Numba rollout and nudge the score by
    # SEARCH_LAMBDA * (sim_value - mean). 0 ⇒ comet_reaper. Time-budgeted; any error falls
    # back to the pure 1-ply score.
    _lam = float(os.environ.get("SEARCH_LAMBDA", "0.0"))
    if _lam != 0.0:
        try:
            ...
            enc = _se.encode_from_tensors(obs_tensors, int(player_count))
            valid_idx = torch.nonzero(cand_valid, as_tuple=False).flatten()
            N = min(int(os.environ.get("SEARCH_TOPN", "30")), int(valid_idx.numel()))
            if N > 0 and (_time.time() - _t0) < _budget:
                order = torch.topk(score[valid_idx], N).indices
                sel = valid_idx[order]
                simv = _se.eval_candidates(
                    enc, pid,
                    cand_src[sel, 0].cpu().numpy(), cand_tgt_slot[sel].cpu().numpy(),
                    cand_send[sel, 0].cpu().numpy(), int(player_count),
                    H=int(os.environ.get("SEARCH_H", "40")),
                )
                simv = simv - simv.mean()
                score = score.clone()
                score[sel] = score[sel] + _lam * torch.tensor(simv, dtype=score.dtype, device=device)
        except Exception as _e:
            import sys as _sys, traceback as _tb
            print("SEARCH_ERR:", repr(_e), file=_sys.stderr)
