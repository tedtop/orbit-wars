...
class ProducerLiteConfig:
    ...
    # --- potential-field: favor targets whose future position is CLOSER to our centroid.
    # Planets orbit; a target rotating toward our cluster is cheaper to hold (less exposure) and
    # harder for enemies to intercept. Bonus ∝ (dist_now - dist_at_arrival) / dist_now.
    # 0 ⇒ disabled. Env POTENTIAL_WEIGHT.
    ...
    *,
    ...
    if _sb != 0.0:
        try:
            ...
            pass
    # schmeekler_potential: bonus for targets rotating TOWARD our centroid by arrival.
    _pw = float(getattr(config, "potential_weight", 0.0))
    if _pw != 0.0:
        try:
            H_pos = movement.x.shape[0] - 1
            if H_pos >= 1:
                if bool(obs.owned.any()):
                    cx = obs.x[obs.owned].mean()
                    cy = obs.y[obs.owned].mean()
                else:
                    cx = torch.tensor(50.0, dtype=dtype, device=device)
                    cy = torch.tensor(50.0, dtype=dtype, device=device)
                tgt_p = cand_tgt_slot.clamp(0, P - 1)          # [C]
                x0 = movement.x[0, tgt_p].to(dtype)
                y0 = movement.y[0, tgt_p].to(dtype)
                d0 = torch.sqrt((x0 - cx) ** 2 + (y0 - cy) ** 2).clamp(min=1.0)
                k_arr = (cand_eta.reshape(C).clamp(min=1.0, max=float(H_pos))
                         .ceil().long().clamp(0, H_pos))        # [C]
                xf = movement.x[k_arr, tgt_p].to(dtype)
                yf = movement.y[k_arr, tgt_p].to(dtype)
                df = torch.sqrt((xf - cx) ** 2 + (yf - cy) ** 2).clamp(min=1.0)
                closing = (d0 - df) / d0                        # +1=fully closed, -1=moved away
                score = score + _pw * closing
        except Exception:
            pass
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        _pw = os.environ.get("POTENTIAL_WEIGHT")
        if _pw is not None:
            config = dataclasses.replace(config, potential_weight=float(_pw))
        row = run_turn(
