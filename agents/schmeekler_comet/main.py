...
class ProducerLiteConfig:
    ...
    # --- comet targeting bonus: actively prioritise capturing ephemeral comet-planets.
    # Field-wide 92% of comets go uncaptured — test whether hunting them improves score.
    # Additive bonus on comet-target candidate scores. 0 ⇒ no preference. Env COMET_TARGET_BONUS.
    comet_target_bonus: float = 1.5
    ...
    *,
    ...
    if _sb != 0.0:
        try:
            ...
            pass
    # comet targeting bonus: nudge toward ephemeral comet-planets — test if hunting them helps.
    _cb = float(getattr(config, "comet_target_bonus", 0.0))
    if _cb != 0.0:
        try:
            # obs_tensors["comet_planet_ids"] is a flat tensor of active comet planet IDs [N]
            # obs_tensors["planets"][:, 0] is the planet_id for each slot index [P]
            comet_ids_tensor = obs_tensors.get("comet_planet_ids", None)
            if comet_ids_tensor is not None and comet_ids_tensor.numel() > 0:
                planet_ids_slot = obs_tensors["planets"][:, 0].long()           # [P] planet_id per slot
                valid_comet_ids = comet_ids_tensor[comet_ids_tensor >= 0].long()  # filter padding (-1)
                if valid_comet_ids.numel() > 0:
                    # is_comet[i] = True if planet_ids_slot[i] is in valid_comet_ids
                    is_comet = torch.isin(planet_ids_slot, valid_comet_ids)      # [P]
                    comet_t = is_comet[cand_tgt_slot.clamp(0, P - 1)]           # [C]
                    score = score + torch.where(comet_t, torch.full_like(score, _cb), torch.zeros_like(score))
        except Exception:
            pass
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        _sb = os.environ.get("SCHMEEKLER_STATIC_BONUS")   # knob sweep via env
        if _sb is not None:
            config = dataclasses.replace(config, static_target_bonus=float(_sb))
        _cb = os.environ.get("COMET_TARGET_BONUS")        # comet bonus sweep via env
        if _cb is not None:
            config = dataclasses.replace(config, comet_target_bonus=float(_cb))
        row = run_turn(
