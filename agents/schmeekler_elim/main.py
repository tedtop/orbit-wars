...
class ProducerLiteConfig:
    ...
    # --- schmeekler (Ted's idea): prefer capturing STATIC (non-rotating) planets first.
    # Static planets (orbital_radius + radius >= 50) sit at the periphery and can't drift
    # toward enemies, so they're the safe land-grab + reliable production base. Additive
    # bonus on static-target candidate scores. 0 ⇒ comet_reaper. Env SCHMEEKLER_STATIC_BONUS.
    static_target_bonus: float = 1.5   # swept: 1.5 beats comet_reaper ~70% 2P; 3+ overcommits and tanks
    # --- elimination mode: when opponent is nearly dead, hard-prioritize finishing them.
    # orbit_lite sometimes takes a "better ROI" neutral planet when the opponent has 2 planets
    # left and you could close the game. That loses tempo. When enemy_planet_count <= threshold:
    #   1. Add elimination_bonus to all enemy planet candidate scores (makes them dominate)
    #   2. Lower effective ROI threshold to elimination_roi_threshold (fire on marginal attacks)
    elimination_threshold: int = 3        # activate when total enemy planets <= this
    elimination_bonus: float = 8.0        # additive bonus to enemy planet scores in elim mode
    elimination_roi_threshold: float = 1.0  # lower ROI bar when closing the game
...
def plan_lite_waves(
    *,
    ...
    # schmeekler: nudge toward STATIC (non-rotating) targets — the safe periphery land-grab.
    _sb = float(getattr(config, "static_target_bonus", 0.0))
    if _sb != 0.0:
        try:
            pl = obs_tensors["planets"]                                          # [P,7]
            cx = pl[:, 2].to(dtype); cy = pl[:, 3].to(dtype); pr = pl[:, 4].to(dtype)
            is_static = (torch.sqrt((cx - 50.0) ** 2 + (cy - 50.0) ** 2) + pr) >= 50.0   # [P]
            static_t = is_static[cand_tgt_slot.clamp(0, P - 1)]                  # [C]
            score = score + torch.where(static_t, torch.full_like(score, _sb), torch.zeros_like(score))
        except Exception:
            pass
    # Elimination mode: when opponent is nearly dead, override scoring to close the game.
    # Counts total enemy planets (all opponents combined) — works in 2P and 4P.
    _elim_roi = float(config.roi_threshold)  # default: unchanged
    _elim_bonus = float(getattr(config, "elimination_bonus", 0.0))
    _elim_threshold = int(getattr(config, "elimination_threshold", 3))
    if _elim_bonus > 0.0:
        try:
            n_enemy_planets = int(
                (obs.alive & (obs.owner_abs >= 0) & (obs.owner_abs != pid)).sum()
            )
            n_my_planets = int((obs.alive & obs.owned).sum())
            # Guard: only fire elim mode when we're clearly dominant (we own more planets
            # than the enemy has). Prevents early-game false positives (opponent starts
            # with 1-2 planets, which would also satisfy the <= 3 count threshold).
            if n_enemy_planets <= _elim_threshold and n_my_planets > n_enemy_planets:
                tgt_owner = obs.owner_abs[cand_tgt_slot.clamp(0, P - 1)]          # [C]
                is_enemy_tgt = cand_valid & (tgt_owner >= 0) & (tgt_owner != pid) # [C]
                score = score + torch.where(
                    is_enemy_tgt,
                    torch.full_like(score, _elim_bonus),
                    torch.zeros_like(score),
                )
                _elim_roi = float(getattr(config, "elimination_roi_threshold", 1.0))
        except Exception:
            pass
    ...
    wave_entries, leftover = _greedy_select(
        ...
        target_exists=target_exists, roi_threshold=_elim_roi,
    )
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        # schmeekler_fmt: format-aware static bonus.
        # In 2P the static grab is strong (opponent can't gang you); in 4P static planets
        # sit exposed at the periphery and draw aggression from multiple enemies.
        # STATIC_BONUS_2P defaults to 1.5 (proven); STATIC_BONUS_4P sweep 0.0→1.0.
        if int(mem.cached_player_count) >= 4:
            _sb4 = os.environ.get("STATIC_BONUS_4P")
            _bonus = float(_sb4) if _sb4 is not None else 0.0   # default: off in 4P
        else:
            _sb2 = os.environ.get("STATIC_BONUS_2P")
            _bonus = float(_sb2) if _sb2 is not None else 1.5   # default: full bonus in 2P
        config = dataclasses.replace(config, static_target_bonus=_bonus)
        row = run_turn(
