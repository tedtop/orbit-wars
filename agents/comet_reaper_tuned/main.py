...
class ProducerLiteConfig:
    ...
    # --- comet_reaper_tuned: re-added 4P leader-attack bonuses (i-m-stronger uses these;
    # comet_reaper dropped them). Additive bonus on enemy-target candidate scores, 4P only.
...
def plan_lite_waves(
    *,
    ...
    # comet_reaper_tuned: 4P leader-attack bonus — nudge scores toward attacking the
    # strongest rival's productive planets (re-added; i-m-stronger uses this). 0 ⇒ off.
    _flb = float(getattr(config, "ffa_leader_attack_bonus", 0.0))
    _ftp = float(getattr(config, "ffa_target_prod_bonus", 0.0))
    if int(player_count) >= 4 and (_flb > 0.0 or _ftp > 0.0):
        try:
            ...
            my_s = strength[pid] if 0 <= pid < A else torch.zeros((), dtype=dtype, device=device)
            tslot = cand_tgt_slot.clamp(0, P - 1)
            towner = owner[tslot]
            enemy_t = (towner >= 0) & (towner != pid) & (~cand_is_def)
            leader_delta = (strength[towner.clamp(0, A - 1)] - my_s).clamp(min=0.0)
            bonus = _flb * leader_delta + _ftp * prod[tslot].to(dtype)
            score = score + torch.where(enemy_t, bonus, torch.zeros_like(score))
        except Exception:
            pass
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        _cj = os.environ.get("CRT_CONFIG")   # Optuna tuning: JSON dict of knob overrides
        if _cj:
            try:
                _ov = {k: v for k, v in json.loads(_cj).items() if hasattr(config, k)}
                config = dataclasses.replace(config, **_ov)
            except Exception:
                pass
        row = run_turn(
