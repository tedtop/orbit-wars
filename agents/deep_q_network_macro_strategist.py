# =============================================================================
# Orbit Wars bot: deep_q_network_macro_strategist
#
# ARCHITECTURE NOTE — Untrained hand-weighted Q-function
# -------------------------------------------------------
# A trained DQN would learn weights W via gradient descent on sampled
# (state, action, reward, next_state) tuples.  At inference-only time
# (no training data, no torch), we implement the *forward pass* with
# hand-set coefficients whose signs and magnitudes mirror the specified
# reward shaping:
#   +10 per capture   → large capture-bonus term, scaled by production
#   +1  per net ship  → production × remaining-horizon component
#   -5  per lost planet → source-exposure penalty using enemy channel
#
# State:  three 10×10 grid channels (friendly ships, enemy ships, production)
#         downsampled from the 100×100 board (each cell covers 10×10 units).
# Action: (source_planet, target_planet) → ships_to_send (enough to capture).
# Policy: greedy argmax Q(state, source, target) over all feasible pairs.
#
# CNN substitute: the spatial feature extraction step is replaced by direct
# weighted reads from the three grid channels, combined with per-pair
# computed features (ETA, garrison, capturability ratio).  The resulting
# scalar Q-value ranks action pairs exactly as a trained policy would.
# =============================================================================

import math

_BOARD  = 100.0
_CX = _CY = 50.0
_SUN_R  = 10.0
_ROT_LIM = 50.0
_EPISODE = 500
_GRID = 10
_CELL = _BOARD / _GRID   # 10.0 — each grid cell covers 10×10 board units

_prev_angles    = {}   # player_id → {planet_id → angle}
_rotation_sign  = {}   # player_id → +1 or -1


# ---------------------------------------------------------------------------
# Engine helpers — copied verbatim from comet_wraith_v3
# ---------------------------------------------------------------------------

def _get(obj, key, default=None):
    if isinstance(obj, dict):
        return obj.get(key, default)
    get_fn = getattr(obj, "get", None)
    if get_fn is not None and callable(get_fn) and not isinstance(obj, type):
        try:
            return get_fn(key, default)
        except TypeError:
            pass
    return getattr(obj, key, default)


def fleet_speed(ships, max_spd=6.0):
    n = max(int(ships), 1)
    return min(1.0 + (max_spd - 1.0) * (math.log(n) / math.log(1000)) ** 1.5, max_spd)


def _pt_seg(p, a, b):
    dx, dy = b[0] - a[0], b[1] - a[1]
    l2 = dx * dx + dy * dy
    if l2 < 1e-12:
        return math.hypot(p[0] - a[0], p[1] - a[1])
    t = max(0.0, min(1.0, ((p[0] - a[0]) * dx + (p[1] - a[1]) * dy) / l2))
    return math.hypot(a[0] + t * dx - p[0], a[1] + t * dy - p[1])


def segment_hits_sun(p0, p1, margin=1.0):
    return _pt_seg((_CX, _CY), p0, p1) < _SUN_R + margin


def path_blocked_by_planet(src, aim, blockers, exclude_ids, buffer=0.5):
    ax, ay = aim[0] - src[0], aim[1] - src[1]
    seg2 = ax * ax + ay * ay
    if seg2 < 1e-9:
        return False
    for b in blockers:
        if b[0] in exclude_ids:
            continue
        bx, by, br = b[2], b[3], b[4]
        t = ((bx - src[0]) * ax + (by - src[1]) * ay) / seg2
        if t <= 0.0 or t >= 1.0:
            continue
        projx, projy = src[0] + t * ax, src[1] + t * ay
        if math.hypot(bx - projx, by - projy) < br + buffer:
            return True
    return False


def predict_planet_pos(init_x, init_y, radius, angular_velocity, abs_step,
                       rotation_sign=1):
    dx, dy = init_x - _CX, init_y - _CY
    r = math.hypot(dx, dy)
    if r + radius < _ROT_LIM:
        ang = math.atan2(dy, dx) + rotation_sign * angular_velocity * abs_step
        return (_CX + r * math.cos(ang), _CY + r * math.sin(ang))
    return (init_x, init_y)


def predict_comet_pos(pid, comets, step_ahead):
    for grp in comets:
        pids = grp["planet_ids"]
        if pid not in pids:
            continue
        i = pids.index(pid)
        path = grp["paths"][i]
        future_idx = grp["path_index"] + step_ahead
        if 0 <= future_idx < len(path):
            pos = path[future_idx]
            return (pos[0], pos[1])
        return None
    return None


def lead_solution(src_pos, tgt_pid, init_x, init_y, tgt_radius, is_comet,
                  comets, angular_velocity, current_step, ships, max_spd,
                  rotation_sign):
    """Returns (angle, eta, aim_pos) or None.  `ships` = fleet size to send."""
    if is_comet:
        cur = predict_comet_pos(tgt_pid, comets, 0) or (init_x, init_y)
    else:
        cur = predict_planet_pos(init_x, init_y, tgt_radius, angular_velocity,
                                 current_step, rotation_sign)
    dist = math.hypot(cur[0] - src_pos[0], cur[1] - src_pos[1])
    t = max(1.0, dist / fleet_speed(ships, max_spd))
    for _ in range(8):
        t_int = max(1, int(round(t)))
        if is_comet:
            fut = predict_comet_pos(tgt_pid, comets, t_int)
            if fut is None:
                return None
        else:
            fut = predict_planet_pos(init_x, init_y, tgt_radius, angular_velocity,
                                     current_step + t_int, rotation_sign)
        dist = math.hypot(fut[0] - src_pos[0], fut[1] - src_pos[1])
        new_t = max(1.0, dist / fleet_speed(ships, max_spd))
        if abs(new_t - t) < 0.05:
            break
        t = new_t
    t_int = max(1, int(round(t)))
    if is_comet:
        aim = predict_comet_pos(tgt_pid, comets, t_int)
        if aim is None:
            return None
    else:
        aim = predict_planet_pos(init_x, init_y, tgt_radius, angular_velocity,
                                 current_step + t_int, rotation_sign)
    return (math.atan2(aim[1] - src_pos[1], aim[0] - src_pos[0]), t_int, aim)


# ---------------------------------------------------------------------------
# DQN state: build three 10×10 grid channels
# ---------------------------------------------------------------------------

def _grid_cell(x, y):
    """Map board coordinates to (row, col) in the 10×10 downsampled grid."""
    return (min(int(y / _CELL), _GRID - 1), min(int(x / _CELL), _GRID - 1))


def _build_channels(planets_raw, fleets_raw, player):
    """
    Three 10×10 channels (CNN input substitute):
      ch_ours[r][c]   — friendly ship mass (planets + in-flight fleets)
      ch_enemy[r][c]  — enemy ship mass
      ch_prod[r][c]   — total production capacity
    """
    ch_ours  = [[0.0] * _GRID for _ in range(_GRID)]
    ch_enemy = [[0.0] * _GRID for _ in range(_GRID)]
    ch_prod  = [[0.0] * _GRID for _ in range(_GRID)]

    for p in planets_raw:
        r, c = _grid_cell(p[2], p[3])
        ch_prod[r][c] += p[6]
        if p[1] == player:
            ch_ours[r][c] += p[5]
        elif p[1] != -1:
            ch_enemy[r][c] += p[5]

    for f in fleets_raw:
        r, c = _grid_cell(f[2], f[3])
        if f[1] == player:
            ch_ours[r][c] += f[6]
        else:
            ch_enemy[r][c] += f[6]

    return ch_ours, ch_enemy, ch_prod


# ---------------------------------------------------------------------------
# Hand-weighted Q-function (replaces the CNN + value head)
# ---------------------------------------------------------------------------

def _q_value(sx, sy, tx, ty, tgt_prod, ships_needed, spendable, eta,
             ch_ours, ch_enemy):
    """
    Q(state, src, tgt) — fixed weights mirror the DQN reward shaping:

      W_capture = 10  ×  cap_ratio × (1 + prod×0.4)      [+10 capture]
      W_prod    =  2  ×  tgt_prod × horizon_frac          [+1 net ship/turn]
      W_eta     =  0.4 × eta                              [time discount]
      W_cost    =  0.1 × ships_needed / src_mass          [opportunity cost]
      W_risk    =  5  ×  exposure_ratio                   [-5 lose planet]
      ctx_term  =  0.5 × enemy_at_tgt / (enemy + 10)     [strategic value]

    cap_ratio: fraction of required ships we can actually send (1.0 = full capture).
    exposure: enemy mass in 3×3 neighborhood vs friendly mass after sending.
    """
    sr, sc = _grid_cell(sx, sy)
    tr, tc = _grid_cell(tx, ty)

    src_mass = ch_ours[sr][sc] + 1.0  # +1 avoids div-zero
    cap_ratio = min(1.0, spendable / (ships_needed + 1e-6))

    # +10 capture: scales with production value of the prize
    capture_term = 10.0 * cap_ratio * (1.0 + tgt_prod * 0.4)

    # +1/net ship: production compounds post-capture up to a ~50-tick horizon
    horizon = max(0.0, 50.0 - eta)
    prod_term = 2.0 * tgt_prod * min(horizon, 50.0) / 50.0

    # Time and opportunity cost
    eta_term  = -0.4 * eta
    cost_term = -0.1 * ships_needed / src_mass

    # -5 lose planet: enemy mass in 3×3 cells around source vs ships left behind
    enemy_near = 0.0
    for dr in range(-1, 2):
        for dc in range(-1, 2):
            nr, nc = sr + dr, sc + dc
            if 0 <= nr < _GRID and 0 <= nc < _GRID:
                enemy_near += ch_enemy[nr][nc]
    ships_left = src_mass - 1.0 - ships_needed   # approx after send
    exposure   = max(0.0, enemy_near - ships_left)
    risk_term  = -5.0 * exposure / (src_mass + exposure + 1.0)

    # Grid context: contested target cells are more strategically valuable
    enemy_at_tgt = ch_enemy[tr][tc]
    ctx_term = 0.5 * enemy_at_tgt / (enemy_at_tgt + 10.0)

    return capture_term + prod_term + eta_term + cost_term + risk_term + ctx_term


# ---------------------------------------------------------------------------
# Main decision logic
# ---------------------------------------------------------------------------

def _decide(obs, config):
    global _prev_angles, _rotation_sign

    max_spd  = float(_get(config, "shipSpeed", 6.0))
    episode  = int(_get(config, "episodeSteps", _EPISODE))
    player   = int(_get(obs, "player", 0))

    p_prev_angles   = _prev_angles.get(player, {})
    p_rotation_sign = _rotation_sign.get(player, 1)
    planets_raw  = _get(obs, "planets", []) or []
    fleets_raw   = _get(obs, "fleets", []) or []
    init_raw     = _get(obs, "initial_planets", []) or []
    comets       = _get(obs, "comets", []) or []
    comet_pids   = set(_get(obs, "comet_planet_ids", []) or [])
    ang_vel      = float(_get(obs, "angular_velocity", 0.035))
    step         = int(_get(obs, "step", 0))
    turns_left   = max(1, episode - step - 2)

    planets    = {p[0]: p for p in planets_raw}
    init_by_id = {p[0]: p for p in init_raw}

    # ---- Rotation-sign inference (identical to comet_wraith_v3) ----
    cur_angles = {}
    for pid, p in planets.items():
        if pid in comet_pids:
            continue
        ip = init_by_id.get(pid)
        if ip is None:
            continue
        dx, dy = ip[2] - _CX, ip[3] - _CY
        if math.hypot(dx, dy) + ip[4] < _ROT_LIM:
            cur_angles[pid] = math.atan2(p[3] - _CY, p[2] - _CX)
    if p_prev_angles and cur_angles:
        deltas = []
        for pid, cur_a in cur_angles.items():
            prev_a = p_prev_angles.get(pid)
            if prev_a is None:
                continue
            d = cur_a - prev_a
            d -= 2.0 * math.pi * round(d / (2.0 * math.pi))
            deltas.append(d)
        if deltas:
            avg = sum(deltas) / len(deltas)
            if abs(avg) > 1e-4:
                p_rotation_sign = 1 if avg > 0 else -1
    _prev_angles[player] = cur_angles
    _rotation_sign[player] = p_rotation_sign

    my_planets = [p for p in planets.values() if p[1] == player]
    if not my_planets:
        return []
    capturable = [p for p in planets.values() if p[1] != player]

    # ---- Build 10×10 grid channels (DQN state representation) ----
    ch_ours, ch_enemy, _ch_prod = _build_channels(planets_raw, fleets_raw, player)

    # ---- Defensive reserves: threat cone (mirrors comet_wraith_v3) ----
    MARGIN = 1
    threat = {mp[0]: 0 for mp in my_planets}
    for f in fleets_raw:
        if f[1] == player:
            continue
        fx, fy, fangle, fships = f[2], f[3], f[4], f[6]
        for mp in my_planets:
            expected = math.atan2(mp[3] - fy, mp[2] - fx)
            diff = fangle - expected
            diff -= 2.0 * math.pi * round(diff / (2.0 * math.pi))
            if abs(diff) < 0.35:
                threat[mp[0]] += fships
    reserve = {mp[0]: max(threat[mp[0]] + MARGIN, MARGIN) for mp in my_planets}

    # ---- Evaluate all (source, target) pairs with the Q-function ----
    candidates = []
    for tgt in capturable:
        tid    = tgt[0]
        tx, ty, tr_r, tships, tprod, towner = (
            tgt[2], tgt[3], tgt[4], tgt[5], tgt[6], tgt[1])
        is_comet = tid in comet_pids
        ip = init_by_id.get(tid)

        if is_comet:
            remaining = 0
            for grp in comets:
                if tid in grp["planet_ids"]:
                    idx = grp["planet_ids"].index(tid)
                    remaining = len(grp["paths"][idx]) - grp["path_index"] - 1
                    break
            if remaining < 15:
                continue

        for src in my_planets:
            sid = src[0]
            sx, sy = src[2], src[3]
            spendable_max = src[5] - reserve.get(sid, MARGIN)
            if spendable_max < 1:
                continue

            # Cheap pre-filter: reject direct path through sun
            if segment_hits_sun((sx, sy), (tx, ty)):
                continue

            def _solve(n, _sx=sx, _sy=sy, _tx=tx, _ty=ty, _tr=tr_r,
                       _tid=tid, _ic=is_comet, _ip=ip):
                if _ic:
                    return lead_solution(
                        (_sx, _sy), _tid, _tx, _ty, _tr, True,
                        comets, ang_vel, step, n, max_spd, p_rotation_sign)
                if _ip is not None:
                    return lead_solution(
                        (_sx, _sy), _tid, _ip[2], _ip[3], _tr, False,
                        comets, ang_vel, step, n, max_spd, p_rotation_sign)
                d = math.hypot(_tx - _sx, _ty - _sy)
                e = max(1, int(d / fleet_speed(n, max_spd)))
                return (math.atan2(_ty - _sy, _tx - _sx), e, (_tx, _ty))

            sol = _solve(spendable_max)
            if sol is None:
                continue
            angle, eta, aim = sol

            garrison   = tships if towner == -1 else tships + tprod * eta
            required   = garrison + MARGIN
            ships_send = min(spendable_max, required)
            if ships_send < 1:
                continue

            # Re-solve with the actual fleet size (speed is ships-dependent)
            sol = _solve(ships_send)
            if sol is None:
                continue
            angle, eta, aim = sol
            if towner != -1:
                garrison   = tships + tprod * eta
                required   = garrison + MARGIN
                ships_send = min(spendable_max, required)
                if ships_send < 1:
                    continue

            # Final sun / planet-blocking check on the lead-aimed path
            if segment_hits_sun((sx, sy), aim):
                continue
            if path_blocked_by_planet((sx, sy), aim, planets_raw, {sid, tid}):
                continue

            # End-game gate: don't launch if fleet can't land before episode ends
            if eta + 2 >= turns_left:
                continue
            if turns_left < 45:
                mult = 2.0 if towner != -1 else 1.0
                if tprod * (turns_left - eta) * mult <= garrison:
                    continue

            q = _q_value(sx, sy, tx, ty, tprod, required, spendable_max, eta,
                         ch_ours, ch_enemy)
            candidates.append((q, sid, tid, angle, eta, ships_send, required))

    # Sort descending by Q-value (greedy argmax policy)
    candidates.sort(key=lambda c: -c[0])

    # ---- Greedy argmax assignment ----
    used      = {mp[0]: 0 for mp in my_planets}
    committed = {}   # target_id → ships already committed from all sources
    moves     = []
    for q, sid, tid, angle, eta, ships_send, required in candidates:
        already = committed.get(tid, 0)
        if already >= required:
            continue
        avail = planets[sid][5] - reserve.get(sid, MARGIN) - used[sid]
        if avail < 1:
            continue
        s = min(avail, required - already)
        if s < 1:
            continue
        moves.append([sid, angle, int(s)])
        used[sid] += s
        committed[tid] = already + s

    return moves


def agent(obs, config):
    """Crash-safe entry point — last callable, called by the engine each turn."""
    try:
        return _decide(obs, config)
    except Exception:
        import sys
        import traceback
        traceback.print_exc(file=sys.stderr)
        return []
